...
def functon_match_v32(ea):
    ...
    # print(ea)
    if mnem is None:
        print('this mnem is None! ea:' + '0x%x'%(ea))
        return False
...
def get_block_info_mips(funcea, block, functions, function_names, func_info, blocks_record, xref_funcptrs, callees):
    '''
    identify func_info of 'call' or 'icall', have 3 cases
    Case 1, jump instruction = ['jalr', 'jr', 'j', 'jal'], target=$t9, exist case of 'icall' when jr $t9
    Case 2, jump instruction belong to ['jalr', 'jr', 'j', 'jal'] and target not be $t9
    Case 3, jump instruction eq to 'bal', no-condition jump
    in all cases, if operator 1 addr(target addr) within the range of extern, it's a lib func call
    otherwise, it's a normal func call
    '''
    bb_info = []
    find_switch = None
    ins_addrs = set()
    ...
    ea = block_start
    ins_addrs.add(ea)
    print('block start: ' + '0x%x'%(ea))
    while ea != BADADDR and ea < block_end:
        # print("Analysis: 0x%x" % (ea))
        # mnem = GetMnem(ea)  # get op
        # opnd0_name = GetOpnd(ea, 1)  # get arg0
        ins = GetDisasm(ea)  # get instruction 
        ins_list = re.split(r'\s[; \s]\s*', ins)  
        if len(ins_list) == 0:
            ea = NextHead(ea)
            if ea in ins_addrs:
                break
            ...
            mnem = ins_list[0]
            opnd0_name = ins_list[1]
        # mnem = ins_list[0]
        # opnd0_name = ins_list[1]
        #if ea == 0x42B934:
        #    print('whz test ins:' + ins)
        #    print('whz test arg0:' + opnd0_name)
        #    print('whz test mnem:' + mnem)
        call_mnems = ['jalr', 'jr', 'j', 'jal']
        if mnem in call_mnems and '$t9' in opnd0_name:
            print("Call: 0x%x %s" % (ea, ins))
            # ins_list = ins.split(" ;")
            # ins_list = re.split(r'\s[; \s]\s*', ins)
            print(ins_list)
            if len(ins_list) >= 3 and '$t9' in ins_list[1]:
                func_name = ins_list[2]
                func_addr = LocByName(func_name)
                if func_addr != BADADDR:
                    if extern_start <= func_addr <= extern_end:
                        # print("  Lib function: %s" % (func_name))
                        func_info['call'].append(('0x%x'%(block_start), '0x%x'%(ea), func_name))
                    else:
                        callees.add(func_addr)
                        # print("  D-Call: 0x%x" % (func_addr))
                        func_info['call'].append(('0x%x'%(block_start), '0x%x'%(ea), '0x%x'%(func_addr)))
                else:
                    # print("  I-Call:")
        ...
        elif mnem in call_mnems:  # example: jal     slprintf
            ...
            if addr != BADADDR and addr != funcea:
                ...
                if attr == 0xc0:
                    ...
                    func_info['call'].append(('0x%x'%(block_start), '0x%x'%(ea), '0x%x'%(opnd0_value)))
        ...
        ea = NextHead(ea)  # get next instruction
        if ea in ins_addrs:
            break
        else:
            ins_addrs.add(ea)
...
def get_all_functions():
    ...
    funcs = set()
    function_count = 0
    # skip non-executable segment, and procedure linkage table
    for segment in segments:
        permissions = getseg(segment).perm
...
def generate_cfg_mips(funcs, function_names, cfg_record, blocks_record, switch_record):
    ...
    for funcea in funcs:
        ...
        function_name = GetFunctionName(funcea)
        try:
            attr = GetFunctionAttr(funcea, 8)  #0x1000 #NO.1#
        except:
            attr = 0
        if attr == 0xc0:
        ...
        for block in flow:
            if ida74:
                func_info['block'].append(('0x%x'%(block.start_ea), '0x%x'%(block.end_ea)))
            else:
                func_info['block'].append(('0x%x'%(block.startEA), '0x%x'%(block.endEA)))
            # print("Block: " % (block.startEA))
            ...
            for succ_block in succ_blocks:
                if ida74:
                    func_info['jmp'].append(('0x%x'%(block.start_ea), '0x%x'%(succ_block.start_ea)))
                    link_blocks.add(succ_block.start_ea)
                else:
                    func_info['jmp'].append(('0x%x'%(block.startEA), '0x%x'%(succ_block.startEA)))
            ...
            if ida74:
                ...
                block_end_ea = block.endEA
			
            if len(succ_blocks) == 0 and block_end_ea != function_obj.end_ea:
                ...
                for addr in all_blocks:
                    if addr not in link_blocks:
                        for switch_block in switch_blocks:
                            if ida74:
                                func_info['jmp'].append(('0x%x'%(switch_block.start_ea),'0x%x'%(addr)))
                            else:
                                func_info['jmp'].append(('0x%x'%(switch_block.startEA), '0x%x'%(addr)))
        ...
        cfg_record['0x'+funcea_name] = func_info
...
try:
    ...
    GetFunctionAttr = GetFunctionAttr
    GetOpType = GetOpType
...
def get_target( attr_list ):
    """
    Obtain the target functions and target basic blocks based on the address 
    of the possible vulnerability point
    """
    hi = "hello"
    tf = []
    tb = []
    for t_attr  in  attr_list: 
        print (hi,t_attr)
        function_obj = get_func(t_attr)
        print(hi, function_obj)
        if ida74 :
            tf.append('0x%x' % function_obj.start_ea)
        else :
            tf.append('0x%x' % function_obj.startEA)
        print (hi,tf)
        flow = FlowChart(function_obj)
        for block in flow:
            if ida74:
                if  block.start_ea <= t_attr < block.end_ea:
                    tb.append('0x%x' % block.start_ea)
                    break
            else:
                if block.startEA <= t_attr < block.endEA:
                    tb.append('0x%x' % block.startEA)
                    break
    #json.dump(cfg_record, open(save_path + file_name + '_cfg.json', 'w'), indent=4)
                    
    f = open(save_path+file_name+'BBtargets.txt', 'a')
    for i in tb :
        f.write(i)
        f.write("\n")
    f.close()
    f = open(save_path+file_name+'Ttargets.txt', 'a')
    for i in tf :
        f.write(i)
        f.write("\n")
    f.close()
...
def main():
    ...
    get_cfg_block_info()
    # here to change the goal/(Vulnerability point list)
    attr_list = [0x409ebc]
    get_target( attr_list )
