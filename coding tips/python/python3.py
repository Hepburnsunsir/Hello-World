...
def readHTML(filepath):
    with open(filepath, 'rb') as f:
        data = re.sub(b"(<!--.*?-->)", b"", f.read(), flags=re.MULTILINE) #去除注释部分
        return data.decode()
    return ''
#与shell交互
def command(cmd):
...
def preprocess():
    '''
    构建载荷; &&第二个命令在第一个执行成功后才执行，｜｜第二个命令总会执行，
    反引号，将一个独立的命令包含在最初命令处理数据中，shell解释器就会执行该命令。
    3*4*5种
    '''
    for q_l in ['', '\'', '"']:
        for q_r in ['', '\'', '"', '#']:
...
#获取仿真固件打开的端口：{'port': '80', 'state': 'open', 'files': [], 'uris': [], 'params': [], 'headers': []}
def get_open_port():
    results = []
...
def get_information():
    ...
    param_map = {}
    ssdp_list = []
    #找到文件名含有cgi的文件，添加到二进制列表；找到文件中包含.cgi结尾的字符串作为uri，含有=的param，含有：的header添加到cgi_map，
    ...
    # get hnap information -P 可以让grep使用perl的正则表达式语法 ,找到soap:Envelope字符串的文件，提取hnap/*xml文件保存到hmap_map
    for line in command("egrep -r 'soap:Envelope' ./{} | grep -P 'hnap/[A-Za-z0-9]+.xml'".format(IID)):
        ...
        with open(filepath, 'rb') as f:
            data = f.read()
            # dlink UTF8 data，b'\xef\xbb\xbf'表示UTF-8编码
            if data[:3] == b'\xef\xbb\xbf':
                data = data[3:].decode()
            else:
                data = data.decode()
            hnap_map[hnap] = data
    #upnp服务 找到文件名带有upnp的文件，搜索：字段，并保存到ssdp_list
    ...
    #找到所有包含action的文件，action 属性规定当提交表单时，向何处发送表单数据
    ...
    for filepath in command('egrep -ril "action=" ./{}'.format(IID)):
        if filepath.find('.htm') != -1:
            try:
                ...
                continue
        # .asp, .aspx (dlink, netgear),为什么要先发送，然后解析响应内容。
...
def get_key_value(key):
    ...
    # make login fail
        ...
        # 'Type': 'WiFi_2.4G', 'DeviceName': 'ZWave-SmartPlug', 'MacAddress': '00:05:5D:01:55:66'
    else:
        return 'default'
...
def fuzz(dummy_info):
    ...
    if dummy_info[0] == 'soap_param':
        ...
        for payload in PAYLOAD_LIST:
            ...
            time.sleep(1)
    elif dummy_info[0] == 'hnap_header': #检查hnap头部命令注入
        for payload in PAYLOAD_LIST:
...
def main():
    ...
    infos = get_open_port() 
    print("[*] ports info")
    print(infos) # {'port': '80', 'state': 'open', 'files': [], 'uris': [], 'params': [], 'headers': []}
